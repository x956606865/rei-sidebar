        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div className="bg-arc-bg text-arc-text border border-black/10 dark:border-white/10 rounded-xl shadow-2xl w-[380px] max-w-[90vw] overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                <div className="p-5 space-y-3">
                    ...
                    {(message || error) && (
                        <div className={`text-xs leading-relaxed rounded-lg px-3 py-2 border ${error ? 'border-red-500/30 bg-red-500/10 text-red-700 dark:text-red-100' : 'border-black/10 dark:border-white/10 bg-arc-hover text-arc-text dark:text-arc-muted'}`}>
                            {error || message}
                        </div>
                    )}
                </div>
